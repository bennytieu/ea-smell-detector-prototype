...
public class MultipleRelationships extends Detector {
    private final List<String> allowedRelationshipTypes;
    private final int maxNumRelationships;

    public MultipleRelationships(List<String> relationshipTypes, int maxNumRelationships) {
        super("Multiple Relationships");
    ...
    public List<EASmell> detect() {
        ...
        while (iterator.hasNext()) {
            ...
            ReferenceableType sourceElements = (ReferenceableType) pair.getKey();
            List<ReferenceableType> targetNodes = (List<ReferenceableType>) pair.getValue();

            if (targetNodes.size() >= maxNumRelationships) {
                String sourceValue = ArchimateModelUtil.getName(sourceElements);
                StringBuilder stringBuilder = new StringBuilder();

                stringBuilder.append("'").append(sourceValue).append("' ")
                             .append("is having multiple relationships with:");
                ...
                addToSmells(new EASmell(getSmellName(), (ElementType) sourceElements, stringBuilder.toString()));
            }
